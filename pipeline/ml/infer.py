...
from pipeline.ml.llm_narrator import narrator
...
# ── Feature Name Mapping (Human Friendly) ───────────────────────────────────
FEATURE_MAP = {
    # Volatility / Risk
    "vol_zscore_5d_tech":         "Tech Volatility Spike",
    "vol_zscore_5d_geopolitical": "Geo-Risk Volatility",
    "vol_zscore_5d_balanced":     "Market Volatility",
    "vol_zscore_5d_conservative": "Defensive Volatility",
    "vol5_tech":                  "5-Day Tech Volatility",
    "vol10_tech":                 "10-Day Tech Volatility",
   
    # Sentiment
    "sentiment_score_financial":  "Global Financial Sentiment",
    "sentiment_score_geopolitical": "Geopolitical Sentiment",
    "sentiment_score_technology":  "Tech Sector Sentiment",
    "sentiment_velocity_financial": "Financial News Momentum",
    "article_spike_geopolitical": "Geopolitical News Surge",
    "article_spike_technology":   "Tech News Surge",
    "weighted_sentiment_tech":    "Sector-Specific Sentiment",
   
    # Fundamental / Correlation
    "intra_corr_tech":            "Tech Stock Correlation",
    "hhi_tech":                   "Tech Sector Concentration",
    "market_regime_flag":         "Market Regime Shift",
}


def get_friendly_name(feat: str) -> str:
    """Return human name from mapping or clean the raw string."""
    if feat in FEATURE_MAP: return FEATURE_MAP[feat]
    return feat.replace("_", " ").title().replace("5D", "5-Day")
...
# ══════════════════════════════════════════════════════════════════════════
# SHAP DRIVER EXTRACTION & CACHING
# ══════════════════════════════════════════════════════════════════════════
...
def extract_shap_drivers(model, row_df: pd.DataFrame, is_classifier: bool = True) -> List[str]:
    """
    Use SHAP to find the top 3 drivers for a single prediction.
    """
    try:
        model_id = id(model)
        if model_id not in SHAP_CACHE:
            if is_classifier:
                SHAP_CACHE[model_id] = shap.TreeExplainer(model)
            else:
                SHAP_CACHE[model_id] = shap.LinearExplainer(model, row_df.values)
       
        explainer = SHAP_CACHE[model_id]


        if is_classifier:
            shap_values = explainer.shap_values(row_df.values)
            ...
            vals = explainer.shap_values(row_df.values)[0]


        # Get top 3 feature indices by absolute contribution
        top_idx = np.argsort(np.abs(vals))[-3:][::-1]
        drivers = [get_friendly_name(row_df.columns[i]) for i in top_idx]
        return drivers
...
def infer_m1_shock_prob(df_feat: pd.DataFrame, portfolio: str) -> dict:
    ...
    # Extract SHAP drivers for M1
    drivers = extract_shap_drivers(model, row, is_classifier=True)


    # TEMPORARY TEST OVERRIDE: Lowered thresholds
    if prob >= 0.35:  # Was 0.75
        signal = "HIGH"
        color  = "danger"
    elif prob >= 0.20:  # Was threshold (approx 0.50)
        signal = "ELEVATED"
        color  = "warning"
    else:
        signal = "NORMAL"
...
def infer_m2_recovery(df_feat: pd.DataFrame, portfolio: str) -> dict:
    ...
    avail = [c for c in feat_cols if c in df_feat.columns]
    row   = df_feat.dropna().tail(1).copy() # Use copy to avoid modifying original
    if row.empty:
        return {"available": False, "reason": "No valid rows for M2 inference"}


    # ── Map dynamic features to model-expected names ─────────────────────
    mapped_row = pd.DataFrame(index=row.index)
   
    # 1. Map Volatility
    target_vol_col = f"vol_zscore_5d_{portfolio}"
    if target_vol_col in row.columns:
        mapped_row["vol_zscore_5d"] = row[target_vol_col]
       
    # 2. Map Article Spikes
    for dom in ["financial", "geopolitical", "technology"]:
        src = f"article_spike_{dom}"
        if src in row.columns:
            mapped_row[f"spike_{dom}"] = row[src]
           
    # 3. Map Market Regime
    if "market_regime_flag" in row.columns:
        mapped_row["market_regime"] = row["market_regime_flag"]
       
    # 4. Map Shock Input
    target_shock_col = f"shock_{portfolio}"
    if target_shock_col in row.columns:
        mapped_row["shock_magnitude"] = row[target_shock_col]
        mapped_row["pre_shock_baseline"] = row[target_shock_col] # Fallback


    # Fill remaining missing features with 0.0
    for c in feat_cols:
        if c not in mapped_row.columns:
            mapped_row[c] = 0.0
           
    final_row = mapped_row[feat_cols]


    p25 = max(1.0, float(q25_model.predict(final_row)[0]))
    p50 = max(p25,  float(q50_model.predict(final_row)[0]))
    p75 = max(p50,  float(q75_model.predict(final_row)[0]))
...
def infer_m3_risk_score(df_feat: pd.DataFrame, portfolio: str) -> dict:
    ...
    # Extract SHAP drivers for M3 (Linear)
    # Note: LinearExplainer needs raw features if they align, or scaled.
    # Here we use row (unscaled names) for extraction.
    drivers = extract_shap_drivers(model, row, is_classifier=False)


    # TEMPORARY TEST OVERRIDE: Lowered thresholds
    if point > 0.012:  # Was 0.025
        risk_label = "HIGH RISK"
        risk_color = "danger"
    elif point > 0.008:  # Was 0.015
        risk_label = "ELEVATED"
        risk_color = "warning"
    else:
        risk_label = "NORMAL"
...
def _derive_buysell(m1: dict, m2: dict, m3: dict, m5: dict, portfolio: str = "tech") -> dict:
    """Synthesis of model signals + LLM narration."""
    ...
    # Combine drivers from M1 and M3
    m1_drivers = m1.get("top_drivers", [])
    m3_drivers = m3.get("top_drivers", [])
    # Unique combined top 3
    combined_drivers = list(dict.fromkeys(m1_drivers + m3_drivers))[:3]


    # TEMPORARY TEST OVERRIDE: Lowered thresholds for Advisory trigger
    if (shock_prob >= 0.30 or risk_label in ("HIGH RISK", "ELEVATED")): # Was shock >= 0.75
        action     = "REDUCE / HEDGE"
        color      = "danger"
        confidence = "High"
        short_reason = f"Shock risk ({shock_prob*100:.0f}%) detected by {combined_drivers[0] if combined_drivers else 'market data'}."
    elif shock_prob < 0.45 and dom_sent > 0.02: # Was shock < 0.35 and sent > 0.05
        action     = "BUY / ADD"
        color      = "success"
        confidence = "Moderate"
        short_reason = f"Low risk + positive {dom_name} sentiment."
    elif shock_prob >= 0.35 or risk_label == "ELEVATED":
        action     = "HOLD / MONITOR"
        color      = "warning"
        confidence = "Moderate"
        short_reason = f"Mixed signals — shock prob {shock_prob*100:.0f}%. Monitor {dom_name} headlines."
    else:
        action     = "HOLD"
        color      = "accent"
        confidence = "Low"
    ...
    ai_justification = narrator.narrate_advisory(
        portfolio_type=portfolio,
        action=action,
        shock_prob=shock_prob,
        risk_level=risk_label,
        top_drivers=combined_drivers
    )


    # Fallback to template if LLM fails
    if not ai_justification:
        if action == "REDUCE / HEDGE":
            reasoning = f"Both M1 ({shock_prob*100:.0f}% shock prob) and M3 ({risk_label}) indicate elevated risk. Top drivers: {', '.join(combined_drivers)}. {recovery}"
        elif action == "BUY / ADD":
            reasoning = f"Low shock probability ({shock_prob*100:.0f}%), improving {dom_name} sentiment ({dom_sent:+.3f}). {recovery}"
        else:
            reasoning = f"Conditions stable. Shock prob {shock_prob*100:.0f}%, Risk: {risk_label}. {recovery}"
    else:
        reasoning = ai_justification + f" {recovery}"
    ...
    return {
        ...
        "reasoning":    reasoning,
        "top_drivers":  combined_drivers,
        "ai_powered":   bool(ai_justification)
    }
...
def run_inference(
    ...
    buysell = _derive_buysell(m1, m2, m3, m4_m5, portfolio=portfolio)
