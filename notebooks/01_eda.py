...
FIGURES_DIR = ROOT_DIR / "reports" / "eda_figures"
...
PANIC_THRESHOLD = -0.10   # sentiment_score below this = "panic" day
...
def fig_return_distributions(pf_ret: pd.DataFrame):
    ...
    for ax, (name, colour) in zip(axes.flat, PORTFOLIO_COLOURS.items()):
        ...
        ax.hist(data, bins=20, color=colour, alpha=0.75, edgecolor="#0d1117", density=True)
...
def fig_panic_detection(df: pd.DataFrame):
    """Q2: How often panic occurs in each news domain."""
    fig, axes = plt.subplots(1, 3, figsize=(16, 5))
    fig.suptitle(f"Sentiment Score Distributions & Panic Detection  (threshold = {PANIC_THRESHOLD})", fontsize=13)
    ...
    for ax, domain in zip(axes, DOMAINS):
        ...
        ax.hist(data, bins=15, color=colour, alpha=0.7, edgecolor="#0d1117", density=True)
        xmin, xmax = data.min(), data.max()
        xs  = np.linspace(xmin, xmax, 200)
        kde = stats.gaussian_kde(data)
        ax.plot(xs, kde(xs), color="white", lw=1.5)

        ax.axvline(PANIC_THRESHOLD, color="#ff6b6b", lw=2, linestyle="--", label=f"Panic < {PANIC_THRESHOLD}")
        ax.axvline(0, color="#8b949e", lw=1, linestyle=":")
...
def fig_correlation_heatmap(df: pd.DataFrame, pf_ret: pd.DataFrame):
    ...
    # Only mask upper triangle of the feature-feature block (keep portfolio-feature block visible)
...
def fig_shock_timeline(df: pd.DataFrame, pf_ret: pd.DataFrame):
    """Q5/Q9: Visualize the prices around biggest single-domain shock day."""
    geo_col = "sentiment_score_geopolitical"
    if geo_col not in df.columns or len(df[geo_col].dropna()) == 0:
        print("  Skipping shock timeline — no geopolitical sentiment data.")
        return

    shock_date = df[geo_col].idxmin()
    window = pd.date_range(
        shock_date - pd.Timedelta(days=7),
        shock_date + pd.Timedelta(days=10),
        freq="B"
    )
    ...
    fig.suptitle(f"Shock Absorption Timeline  (shock date: {shock_date.date()})", fontsize=13)

    # Upper: portfolio returns around shock
    for name, colour in PORTFOLIO_COLOURS.items():
        ...
        ax1.plot(series.index, series.values, marker="o", markersize=4,
                 color=colour, label=name.capitalize(), linewidth=1.8)

    ax1.axvline(shock_date, color="#ff6b6b", lw=2, linestyle="--", label="Shock day")
    ax1.axhline(0, color="#8b949e", lw=0.8, linestyle=":")
    ax1.set_ylabel("Portfolio Log Return")
    ax1.legend(fontsize=9)
    ax1.grid(True, alpha=0.3)
    ax1.set_title("Portfolio Returns Around Shock")

    # Lower: geopolitical sentiment
    sent_window = df.loc[df.index.isin(window), geo_col]
    ax2.bar(sent_window.index, sent_window.values,
            color=[DOMAIN_COLOURS["geopolitical"] if v >= 0 else "#e74c3c"
                   for v in sent_window.values],
            alpha=0.8, width=0.8)
    ax2.axvline(shock_date, color="#ff6b6b", lw=2, linestyle="--")
    ax2.axhline(0, color="#8b949e", lw=0.8, linestyle=":")
    ax2.set_ylabel("Geo Sentiment Score")
    ax2.set_xlabel("Date")
    ax2.grid(True, alpha=0.3)
    ax2.set_title("Geopolitical Sentiment")
...
def fig_gold_vs_geopolitical(df: pd.DataFrame):
    ...
    if geo_col not in df.columns or gold_col not in df.columns:
        print("  Skipping gold vs geo — columns missing.")
        return

    combined = df[[geo_col, gold_col]].dropna()
    if len(combined) < 3:
        print("  Not enough data for gold vs geo plot.")
        return
    ...
    # Scatter coloured by intensity of negative geo sentiment
...
def fig_tech_overreaction(df: pd.DataFrame, pf_ret: pd.DataFrame):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sc = ax.scatter(x, y,
                    c=["#e74c3c" if v < 0 else "#2ecc71" for v in x],
                    s=80, alpha=0.8, edgecolors="#0d1117", lw=0.5)

    # Annotate spike days
    spike_threshold = np.percentile(np.abs(x), 80)
    for i, (xi, yi) in enumerate(zip(x, y)):
        if abs(xi) >= spike_threshold:
            ax.annotate(
                combined.index[i].strftime("%b %d"),
                (xi, yi), fontsize=7, color="white",
    ...
    if len(x) > 2:
        ...
        xline = np.linspace(x.min(), x.max(), 100)
        r, p_val = stats.pearsonr(x, y)
        ax.plot(xline, m * xline + b, color="#4a90d9", lw=2,
                label=f"Regression  (r={r:.3f}, p={p_val:.3f})")

    ax.axvline(0, color="#8b949e", lw=0.8, linestyle=":")
    ax.axhline(0, color="#8b949e", lw=0.8, linestyle=":")
    ax.set_title("Tech Portfolio: Do Returns Overreact to Tech Sentiment Spikes?\n"
...
def fig_lag_analysis(df: pd.DataFrame, pf_vol: pd.DataFrame):
    """Q7: Is there a measurable lag between news intensity and volatility?"""
    # Use article_count as proxy for news intensity
    intensity_cols = {
        d: f"article_count_{d}" for d in DOMAINS
        if f"article_count_{d}" in df.columns
    }

    max_lag = min(5, len(df) - 2)
    fig, axes = plt.subplots(len(PORTFOLIOS), 1,
                             figsize=(12, 4 * len(PORTFOLIOS)), sharex=True)
    ...
    for ax, (pf_name, colour) in zip(axes, PORTFOLIO_COLOURS.items()):
        ...
        for domain, col in intensity_cols.items():
            ...
            for lag in lags:
                combined = pd.concat(
                    [df[col].shift(lag), vol_series], axis=1
                ).dropna()
                if len(combined) < 3:
                    corrs.append(np.nan)
                    continue
                ...
                corrs.append(r)
            ax.plot(list(lags), corrs, marker="o", markersize=5,
                    color=DOMAIN_COLOURS[domain], label=domain.capitalize(), lw=1.8)
...
def fig_predictive_power(df: pd.DataFrame, pf_ret: pd.DataFrame):
    """Q8: Which information source has the highest predictive power?"""
    # Use lag-1 sentiment features vs next-day portfolio returns
    lag_cols = {
        d: f"lag1_sentiment_score_{d}" for d in DOMAINS
        if f"lag1_sentiment_score_{d}" in df.columns
    }

    results = []
    for pf_name in PORTFOLIOS:
        if pf_name not in pf_ret.columns:
            continue
    ...
    if not results:
        print("  Skipping predictive power — insufficient data.")
        return
    ...
    for i, (col, dom_colour) in enumerate(
        zip(pivot.columns, [DOMAIN_COLOURS.get(c.lower(), "#888") for c in pivot.columns])
    ):
        ax.bar(x + i * w - 0.4 + w / 2, pivot[col], width=w,
               color=dom_colour, alpha=0.8, label=col, edgecolor="#0d1117")

    ax.set_xticks(x)
    ax.set_xticklabels(pivot.index)
    ax.set_ylabel("|Pearson r|  (lag-1 sentiment → next-day return)")
    ax.set_xlabel("Portfolio")
    ax.set_title("Predictive Power of Each News Domain\n(absolute correlation of yesterday's sentiment with today's return)", fontsize=12)
...
def fig_stabilisation_time(df: pd.DataFrame, pf_vol: pd.DataFrame):
    ...
    geo_col = "sentiment_score_geopolitical"
    if geo_col not in df.columns or len(pf_vol.dropna()) < 5:
        return

    shock_date = df[geo_col].idxmin()
    post_shock = pf_vol.loc[pf_vol.index >= shock_date].head(12)

    if len(post_shock) < 3:
        print("  Not enough post-shock days for stabilisation analysis.")
        return

    # Normalise: vol on shock day = 1.0
    ...
    for name, colour in PORTFOLIO_COLOURS.items():
        ...
        ax.plot(range(len(series)), series.values, marker="o", markersize=5,
                color=colour, label=name.capitalize(), linewidth=2)
    ...
    ax.set_title(f"Time to Stabilise After Shock  (shock: {shock_date.date()})\n"
                 "Normalised Rolling Volatility  (1.0 = vol at shock day)", fontsize=12)
...
def run():
    print("=" * 60)
    print("  EDA  |  Portfolio Risk Analytics")
    print("=" * 60)
    ...
    print("=" * 60)
    print("  EDA COMPLETE")
    print(f"  All 11 figures saved to:  {FIGURES_DIR.resolve()}")
    print("=" * 60)
